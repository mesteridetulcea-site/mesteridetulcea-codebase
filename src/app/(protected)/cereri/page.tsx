...
export default async function MesterCereriPage() {
  ...
  const total = cereri.length + transportCereri.length
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      {/* ══════════════════════════════════════════════
          HERO — identic cu /cauta, suprapus cu navbar
      ══════════════════════════════════════════════ */}
        ...
        <div className="container relative z-10 flex flex-col items-center text-center pt-[96px] pb-14">
          ...
          <p className="mt-4 font-condensed text-[11px] tracking-[0.22em] uppercase text-white/32">
            {total === 0
              ...
              : `${total} ${total === 1 ? "cerere" : "cereri"} disponibile`}
          </p>
        </div>
      </section>
              ...
              {transportCereri.length > 0 && (
                <div>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <span className="font-condensed text-[11px] tracking-[0.22em] uppercase text-[#584528]/55">
                      {transportCereri.length} {transportCereri.length === 1 ? "cerere transport" : "cereri transport"}
                    </span>
                  </div>
                    ...
                    {transportCereri.map((tr) => {
                      ...
                      return (
                        <Link key={tr.id} href={`/cereri/transport/${tr.id}`}>
                          <article className="bg-white group hover:bg-[#fdfcf9] transition-colors duration-200">
                            <div className="px-7 pt-6 pb-5">
                              ...
                              <div className="flex items-start gap-3 mb-3">
                                <div className="flex flex-col items-center gap-1 pt-0.5 shrink-0">
                                  ...
                                  <div className="w-px h-4 bg-[#584528]/15" />
                              ...
                              {tr.description && (
                                <p className="text-sm text-[#3d2e14]/55 leading-relaxed line-clamp-2 mb-3">
                                  {tr.description}
                                </p>
                              )}

                              <div className="flex items-center justify-between">
                                <time className="flex items-center gap-1.5 font-condensed text-[10px] tracking-[0.14em] uppercase text-[#584528]/30">
                                  <Clock className="h-3 w-3" />
                                  {formatDistanceToNow(new Date(tr.created_at), { addSuffix: true, locale: ro })}
                                </time>
                                <span className="flex items-center gap-1.5 font-condensed text-[10px] tracking-[0.18em] uppercase text-[#584528]/38 group-hover:text-primary transition-colors duration-200">
                                  Detalii <ArrowUpRight className="h-3 w-3" />
                                </span>
                              </div>
                            </div>
